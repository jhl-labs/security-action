...
class NativeAuditScanner(BaseScanner):
    ...
    def _run_pip_audit(self) -> list[Finding]:
        ...
        # requirements 파일 찾기
        req_files = (
            list(workspace_path.glob("**/requirements*.txt"))
        ...
        try:
            ...
            if result.stdout:
                try:
                    audit_data = json.loads(result.stdout)
                    findings.extend(self._parse_pip_audit(audit_data))
                except json.JSONDecodeError as e:
    ...
    def _parse_pip_audit(self, data: list) -> list[Finding]:
        ...
        for vuln in data:
            ...
            for v in vuln.get("vulns", []):
                ...
                finding = Finding(
                    ...
                    file_path=self.normalize_path("requirements.txt"),
                    line_start=1,
                    metadata={
                        "package": pkg_name,
                        "version": version,
