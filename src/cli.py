...
Examples:
  ...
  # Self-hosted OpenAI 호환 엔드포인트 사용
  python -m cli --ai-review --ai-provider openai --ai-model gpt-4o-mini \
    --openai-base-url http://llm-gateway.local/v1 --openai-api-key $OPENAI_API_KEY

  # 설정 파일 사용
  python -m cli --config .security-action.yml

  # SARIF를 GitHub Security로 직접 업로드
  python -m cli --sarif security-results.sarif --upload-sarif --sarif-category security-action
        """,
    )
    ...
    ai_group.add_argument(
        ...
        default="auto",
        help="AI 제공자 선택 (기본: auto)",
    )
    ai_group.add_argument(
        "--ai-model",
        help="AI 모델명 (예: gpt-4o, claude-3-5-sonnet-20241022)",
    )
    ai_group.add_argument(
        "--openai-api-key",
        help="OpenAI API 키",
    ...
    ai_group.add_argument(
        "--openai-base-url",
        help="OpenAI 호환 API Base URL (사내 게이트웨이/프록시)",
    )
    ai_group.add_argument(
        "--anthropic-api-key",
    ...
    output_group.add_argument(
        "--upload-sarif",
        action="store_true",
        help="생성한 SARIF를 GitHub Security로 직접 업로드",
    )
    output_group.add_argument(
        "--sarif-category",
        default="security-action",
        help="SARIF 업로드 카테고리(기본: security-action)",
    )
    output_group.add_argument(
        "--fail-on-sarif-upload-error",
        action="store_true",
        help="SARIF 업로드 실패 시 종료 코드 1 반환",
    )
    output_group.add_argument(
        "--json",
        dest="json_output",
...
def main() -> int:
    ...
    os.environ["INPUT_AI_PROVIDER"] = args.ai_provider
    ...
    if args.openai_base_url:
        os.environ["INPUT_OPENAI_BASE_URL"] = args.openai_base_url
